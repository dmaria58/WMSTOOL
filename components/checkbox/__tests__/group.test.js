...
describe('CheckboxGroup', () => {
  it('should work basically', () => {
    ...
    wrapper.find('.wmstool-checkbox-input').at(0).simulate('change');
    expect(onChange).toBeCalledWith(['Apple']);
    wrapper.find('.wmstool-checkbox-input').at(1).simulate('change');
    expect(onChange).toBeCalledWith(['Apple', 'Pear']);
    wrapper.find('.wmstool-checkbox-input').at(2).simulate('change');
  ...
  it('does not trigger onChange callback of both Checkbox and CheckboxGroup when CheckboxGroup is disabled', () => {
    ...
    groupWrapper.find('.wmstool-checkbox-input').at(0).simulate('change');
    expect(onChangeGroup).not.toBeCalled();
    groupWrapper.find('.wmstool-checkbox-input').at(1).simulate('change');
    expect(onChangeGroup).not.toBeCalled();
  });
