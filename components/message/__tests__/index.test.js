...
describe('message', () => {
  ...
  it('should be able to config top', () => {
    ...
    expect(document.querySelectorAll('.wmstool-message')[0].style.top).toBe('100px');
  });
  ...
  it('should be able to hide manually', () => {
    ...
    expect(document.querySelectorAll('.wmstool-message-notice').length).toBe(2);
    hide1();
    jest.runAllTimers();
    expect(document.querySelectorAll('.wmstool-message-notice').length).toBe(1);
    hide2();
    jest.runAllTimers();
    expect(document.querySelectorAll('.wmstool-message-notice').length).toBe(0);
  });

  it('should be able to destroy globally', () => {
    message.info('whatever', 0);
    message.info('whatever', 0);
    expect(document.querySelectorAll('.wmstool-message').length).toBe(1);
    expect(document.querySelectorAll('.wmstool-message-notice').length).toBe(2);
    message.destroy();
    expect(document.querySelectorAll('.wmstool-message').length).toBe(0);
    expect(document.querySelectorAll('.wmstool-message-notice').length).toBe(0);
  });
  ...
  // https://github.com/wmstool-design/wmstool-design/issues/8201
